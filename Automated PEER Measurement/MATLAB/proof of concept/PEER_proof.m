...
for i = 5:5
    ...
    for k = 1:2
        ...
        maxBrtVal(i,k) = drawShape(img_TK_NC{i,k}, tumorArea(i,1), closestDist, rotAngle, centx, centy, 'b', k, h(i));
        h1 = gcf;
        maxBrtValC(i,k) = drawShape(img_TK_C{i,k}, tumorAreaC(i,1), closestDistC, rotAngleC, centxC, centyC, 'b', k, h(i+5));
        h2 = gcf; 
    end
    %% salculating PEER
...
PEER_RMSE = sqrt(mean((PEER-PEER_man{1,2}).^2))
...
function maxBrtValAct = drawShape(img, tumorArea, closestDist, rotAngle, centx, centy, color, K, h)
